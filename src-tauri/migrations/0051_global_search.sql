...
CREATE TRIGGER search_attachment_text_reset AFTER UPDATE ON attachments BEGIN
 DELETE FROM search_file_state WHERE document_id='attachment:' || NEW.id;
END;
CREATE TRIGGER search_artifact_text_reset AFTER UPDATE ON artifacts BEGIN
...
SELECT 'capability-event:' || sequence,'execution','capability-event',CAST(sequence AS TEXT),
 ...
 COALESCE(json_extract(payload_json,'$.occurredAt'),''),caller_window FROM capability_events;
CREATE TRIGGER search_capability_event_insert AFTER INSERT ON capability_events BEGIN
 INSERT INTO search_documents(id,kind,source,target_id,title,body,updated_at,owner_window)
...
END;
CREATE TRIGGER search_capability_event_delete AFTER DELETE ON capability_events BEGIN
