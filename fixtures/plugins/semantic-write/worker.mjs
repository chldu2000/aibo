...
createInterface({ input: process.stdin }).on('line', line => {
  ...
  if (method === 'capability.initialize') {
    ...
    if (p.operationId === 'dev.aibo.semantic-write.view' || p.capability === 'dev.aibo.semantic-write.view') {
      let content = ''; try { content = readFileSync(path.join(p.context.workspacePath, 'effect.txt'), 'utf8'); } catch {}
      let mode = 'normal'; try { mode = readFileSync(path.join(p.context.workspacePath, 'mode.txt'), 'utf8').trim(); } catch {}
      reply(id, { invocationId: p.invocationId, generationId: p.generationId, output: {
        state: { status: 'ready', message: '' },
        view: { kind: 'detail', itemId: 'effect.txt', properties: [{ label: '文件', value: 'effect.txt' }], content, truncated: false },
        actions: [{ id: 'refresh', label: '刷新', intent: 'refresh', enabled: true },
          { id: 'dev.aibo.semantic-write.append', label: '写入样例', intent: 'execute', enabled: true, input: { value: 'semantic-write', mode } }]
      } }); return;
    }
    ...
    setTimeout(() => reply(id, { invocationId: p.invocationId, generationId: p.generationId,
      output: p.input.mode === 'invalid-output' ? { invalid: true } : { value: p.input.value, caller: p.context.originalCaller.id, permissions: p.context.permissions, chainLength: p.context.callChain.length } }), p.input.mode === 'slow' ? 5000 : p.input.mode === 'delayed' ? 500 : 0);
  }
});
