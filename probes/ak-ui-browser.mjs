...
try{
 ...
 for(const mode of ['light','dark']){
  ...
  const titlebarToggle=page.locator('.window-actions .ui-button[aria-pressed]');
  ...
  const ordinary=page.locator('.sidebar-footer .ui-button').first();
 ...
 assert.equal(await page.locator('.appearance-kit-info').count(),0);
 assert.equal(await page.locator('.appearance-kit-option').count(),2);
 assert.equal(await page.locator('.management-content').evaluate(e=>e.scrollTop),0);
 assert.equal(await page.getByRole('radiogroup',{name:'主题'}).count(),1);
 ...
 await page.keyboard.press('Escape');assert.equal(await page.getByRole('dialog',{name:'工作台设置',exact:true}).count(),0);
