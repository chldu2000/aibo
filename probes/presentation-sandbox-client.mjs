...
window.sandboxProbe={intents,failures,get savedState(){return viewState.read({workspaceId:'workspace',sessionId:'session',revision:0});},get recovered(){return recovered;},
  ...
  async mount(source,data={}) {
    const candidate=await preparePresentationSandbox(target,await packageOf(source),input(data),intent=>{intents.push(intent);if(echoDraft&&intent.id==='draft')current?.update(input({draft:intent.value}));},error=>failures.push(error.message),undefined,{viewState,localInputActions:echoDraft?['draft']:[]});
    current?.dispose();current=candidate;candidate.activate();
  },
  update(data){current.update(input(data));},
  updateScope(workspaceId,sessionId,data){const next=input(data);current.update({...next,context:{...next.context,workspaceId,sessionId}});},
