...
try{
 ...
 assert.equal(await page.getByRole('textbox',{name:'槽位草稿'}).inputValue(),'SLOT_DRAFT');
 ...
 await page.evaluate(()=>window.lifecycleProbe.old());
 assert.equal((await page.evaluate(()=>window.lifecycleProbe.state())).actions,1);
 assert.equal(await page.getByRole('textbox',{name:'槽位草稿'}).inputValue(),'');
 await page.evaluate(()=>window.lifecycleProbe.select('a'));
 await page.waitForFunction(()=>document.querySelector('label input')?.value==='SLOT_DRAFT');
 ...
 for (const kit of ['shadcn', 'material3']) {
  ...
  await page.getByRole('button', {name:'导航', exact:true}).focus();
  await page.evaluate(() => window.lifecycleProbe.switch('focus'));
  assert.equal(await page.evaluate(() => document.activeElement?.dataset.presentationFocus), 'draft', 'removed navigation focus must move to a usable control');
  ...
  await page.getByRole('complementary', {name:'槽位导航'}).waitFor();
  for (const unavailable of ['disabled', 'hidden']) {
   await page.evaluate(() => window.lifecycleProbe.draftAvailability(false, false));
   await page.getByRole('textbox', {name:'草稿', exact:true}).focus();
   await page.evaluate(unavailable => window.lifecycleProbe.draftAvailability(unavailable === 'disabled', unavailable === 'hidden'), unavailable);
   await page.evaluate(() => window.lifecycleProbe.switch('review'));
   assert.equal(await page.evaluate(() => {
    const active = document.activeElement;
    return active instanceof HTMLButtonElement && !active.disabled && active.getClientRects().length > 0 && Boolean(active.closest('.workbench-presentation'));
   }), true, `${unavailable} remembered control must not strand keyboard focus`);
 ...
 const result={removedControlFocusRecovered:true,disabledControlSkipped:true,hiddenControlSkipped:true,tabContinues:true,namedSlotsBothSkins:true,focusOmitsNavigationAndAuxiliary:true,overlaysReachable:true,hostSlotDraftSurvivesRemount:true,hostSlotDraftSessionIsolation:true,failedMountFallback:true,streamContinues:true,draft:true,semanticFocus:true,oldGenerationRejected:true,oldSessionRejected:true,currentActions:true};
