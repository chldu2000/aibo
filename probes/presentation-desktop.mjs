...
execFileSync(process.execPath,[path.join(built.root,'node_modules/@aibo/presentation-shadcn/build.mjs'),upgrade,'0.3.1'],{cwd:built.root,stdio:'pipe'});
...
const manifest=JSON.parse(await readFile(path.join(brokenSource,'presentation.json'),'utf8'));manifest.version='0.3.2';await writeFile(path.join(brokenSource,'source.json'),JSON.stringify(manifest));const broken=path.join(root,'broken');await buildPresentation(path.join(brokenSource,'source.json'),broken);
...
const runtimeManifest=JSON.parse(await readFile(path.join(runtimeSource,'presentation.json'),'utf8'));runtimeManifest.version='0.3.3';await writeFile(path.join(runtimeSource,'source.json'),JSON.stringify(runtimeManifest));const runtimeFault=path.join(root,'runtime-fault');await buildPresentation(path.join(runtimeSource,'source.json'),runtimeFault);
