...
try {
  ...
  if (config.stage === 0) {
    ...
    saved = { workspaceId: workspace.id, gitId: git.id, viewId: view.id };
  ...
  for (const [kit, title] of [['shadcn', 'Git 工作区变更'], ['material3', 'Git 工作区变更'], ['material3', 'Git 变更（独立声明包）']]) {
    ...
    } else { button('返回变更列表').click(); await until(() => document.querySelector('button[aria-label="查看差异 native.txt"]'), 'back'); button('关闭工作区工具').click(); await until(() => !document.querySelector('section.installed-workbench'), 'close'); }
  }
  if (config.stage === 1) {
  ...
  await report({ ok: true, stage: config.stage, saved, evidence: { backgroundGitWithoutUi: true, installedCommands: rendered, disablingViewClosesSurface: true, noAgentSession: true, ...(config.stage === 1 ? { actualAppRestart: true, persistedCapabilityBinding: true, uninstallInvalidatesBothForms: true } : {}) } });
