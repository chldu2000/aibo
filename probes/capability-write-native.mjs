...
await mkdir(workspacePath); await cp('fixtures/plugins/capability-write', packagePath, { recursive: true });
...
await writeFile(config, JSON.stringify({ identifier, productName: 'Aibo isolated capability writes', build: { beforeDevCommand: '', devUrl: `http://127.0.0.1:${server.httpServer.address().port}` }, app: { windows: [{ label: 'main', title: 'Capability writes', url: 'probes/capability-write-native.html', width: 900, height: 700 }] } }));
try {
  ...
  const nativeClicks = await Promise.all(clicks); if (nativeClicks.length !== 3) throw Error('Expected one denial and two approvals');
  if (await readFile(path.join(workspacePath, 'effect.txt'), 'utf8') !== 'native-once\nnative-cancel\n') throw Error('Write effects differ or were replayed');
  if (await access(path.join(workspacePath, 'late.txt')).then(() => true, () => false)) throw Error('Cancelled descendant continued writing');
  if (await readFile(path.join(root, 'started'), 'utf8') !== 'started\nstarted\n') throw Error('Unexpected runtime activation');
  ...
  await writeFile('/tmp/aibo-p4-capability-write-native.json', JSON.stringify(evidence, null, 2) + '\n'); console.log(JSON.stringify(evidence));
