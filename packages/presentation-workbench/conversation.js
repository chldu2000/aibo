...
export function renderConversation(state,actions){
 ...
 if(state.session){
  ...
  if(slash||state.agentCommands.length)composer.push(slash?node('div','conversation:commands',null,[node('div','commands:categories',null,groups.map(group=>node('button','commands:category:'+group.id,group.label+' ('+group.commands.length+')',[],{type:'button',role:'tab','aria-selected':String(group.id==='all')})),{role:'tablist','aria-label':'命令分类'}),...commandButtons],{role:'listbox','aria-label':'命令建议'}):node('details','conversation:commands',null,[node('summary','commands:title','命令'),...commandButtons]));
  const suggestionButtons=mention?pathButtons:slash?commandButtons:[];
  if(suggestionButtons.length||slash&&groups.length)draftField.children[1].suggestions={listKey:mention?'conversation:paths':'conversation:commands',keys:suggestionButtons.filter(button=>button.events?.click).map(button=>button.key),confirmWithTab:mention,confirmWithPrimary:mention,...(slash&&!mention?{categories:groups.map(group=>({key:'commands:category:'+group.id,options:group.commands.filter(command=>find('selectCommand',command.name)).map(command=>'command:'+command.name)}))}:{})};
