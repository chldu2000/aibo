...
export function renderConversation(state,actions){
 ...
 if(state.session){
  ...
  const groups=slash?[['all','All'],['agent','Agent'],['skill','Skills'],['extension','Extension']].map(([id,label])=>({id,label,commands:matches.filter(command=>id==='all'||categoryOf(command)===id)})):[];
