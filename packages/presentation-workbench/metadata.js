...
const labels={interactionMode:{ask:'问答',plan:'计划',edit:'编辑'},filesystemPolicy:{'read-only':'只读','workspace-write':'工作区可写','danger-full-access':'完整文件访问'},commandPolicy:{disabled:'禁用',approved:'需审批',trusted:'自动执行'},networkPolicy:{disabled:'禁用','agent-managed':'Agent 管理'},approvalPolicy:{never:'从不请求审批',untrusted:'未信任操作需审批','on-request':'按请求审批',trusted:'信任模式'}};
export function renderExecutionProfile(profile,key){
 if(!profile)return null;
 const fields=[['interactionMode','模式'],['filesystemPolicy','文件'],['commandPolicy','命令'],['networkPolicy','网络'],['approvalPolicy','审批'],['model','模型'],['reasoningEffort','推理强度']];
