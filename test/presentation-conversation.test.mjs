const controls = ids => ids.map(id=>({id,kind:'permission',label:'Plugin '+id,description:'Declared option',profile:{filesystemPolicy:'read-only'}}));
...
test('access actions use host-authorized modes and never infer authority from an agent name', () => {
  for (const agent of ['codex','pi','external.agent']) {
    ...
    assert.deepEqual(modes({...state,session,executionProfile:{...state.executionProfile,sessionControls:controls(['custom-read'])}}),['custom-read']);
