...
test('packed protocol compiles and imports outside the repository without DOM or framework dependencies', async () => {
  ...
  try {
    ...
    assert.ok(packed.files.every(file=>file.path==='package.json'||file.path==='README.md'||/^dist\/(?:index|semantic|presentation|renderer|capability|session|presentation-package|presentation-runtime)\.(?:js|d\.ts)$/.test(file.path)), 'archive only contains public built contracts');
...
import type { JsonValue, PresentationPackageManifest, PresentationNode, PresentationInput } from '@aibo/plugin-protocol';
