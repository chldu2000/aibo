...
test('packed protocol compiles and imports outside the repository without DOM or framework dependencies', async () => {
  ...
  try {
    ...
    const sdkSource=path.resolve('packages/capability-sdk'),sdkStaging=path.join(root,'sdk');
    ...
    const sdkInstalled=path.join(consumer,'node_modules/@aibo/capability-sdk');await mkdir(sdkInstalled,{recursive:true});
...
import { createCapabilityRuntime } from '@aibo/capability-sdk';
