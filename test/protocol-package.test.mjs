...
test('packed protocol compiles and imports outside the repository without DOM or framework dependencies', async () => {
  ...
  try {
    ...
    assert.ok(packed.files.every(file=>file.path==='package.json'||file.path==='README.md'||/^dist\/(?:index|semantic|presentation|renderer|capability|session|presentation-package|presentation-runtime|presentation-controls|presentation-navigation|presentation-conversation|presentation-git|presentation-inspector)\.(?:js|d\.ts)$/.test(file.path)), 'archive only contains public built contracts');
...
import type { JsonValue, PresentationPackageManifest, PresentationNode, PresentationInput, PresentationControlData, PresentationNavigationAction, PresentationConversationAction, PresentationGitAction, PresentationInspectorAction } from '@aibo/plugin-protocol';
...
const conversationAction: PresentationConversationAction = {token:'conversation:1',operation:'send',event:'click',args:[]};
const gitAction: PresentationGitAction = {token:'git:1',operation:'openDiff',event:'click',args:['file','staged']};
const inspectorAction: PresentationInspectorAction = {token:'inspector:1',operation:'showDiff',event:'click',args:['turn','file']};
