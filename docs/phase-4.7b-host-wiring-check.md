...
> 状态：自动化检查通过；Tauri 窗口人工 smoke 待执行
> 日期：2026-09-08
> 范围：App 接线、插件工作台状态、进程退出分类、会话终态和恢复

## 结论

宿主集成测试、旧数据库迁移测试、App 接线静态门禁和关键异常回归均已通过。P4.7B 仍不能宣称全部验收完成：真实 Tauri 窗口中的点击、键盘焦点、窄窗口和两套皮肤切换需要在可交互桌面环境执行一次人工 smoke。
...
人工 smoke 通过前，P4.7B 状态保持“自动化检查通过；人工 smoke 待执行”；P4.7D 再补充完整视图动作、窄窗口、焦点恢复和生命周期验收。
