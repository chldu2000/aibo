...
用户在创建会话或权限设置中选择执行配置时，即授权该会话按此配置执行顶层轮次；
完全文件访问、免审批写入或免审批命令执行不再弹出额外的宽泛权限确认。配置持久化后，
运行时重启、应用重启及插件重新加载均继续使用所选配置。具体工具仍执行所选审批策略，
“请求审批”模式仍会对 Agent 发起的具体高风险操作显示审批请求。
