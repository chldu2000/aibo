...
> 状态：实施中（P4.7A 已完成；P4.7B 自动化检查通过，人工 smoke 待执行）
...
| P4.7B 最小外部插件闭环 | 自动化检查通过；人工 smoke 待执行 | Registry、Supervisor、统一会话 API、最小 Renderer、本地安装；现有 Manager 暂经内部适配层路由 | 仓库外测试插件无需修改或重编译宿主即可安装、交互、取消、恢复并显示标准视图；据此估算余量 |
...
P4.7B 的宿主接线和异常场景检查见 [Phase 4.7B 检查记录](phase-4.7b-host-wiring-check.md)。自动化门禁已通过；Tauri 窗口人工 smoke 完成前不关闭本切片。
