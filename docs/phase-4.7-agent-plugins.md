...
| 切片 | 交付 | 退出条件 |
| --- | --- | --- |
| P4.7A 契约与设计 | ADR、清单、运行时/视图 schema、能力与错误模型、binding 迁移设计 | 有可回放的请求/事件/视图样例，明确版本与权限边界 |
| P4.7B 最小外部插件闭环 | Registry、Supervisor、统一会话 API、最小 Renderer、本地安装；现有 Manager 暂经内部适配层路由 | 仓库外测试插件无需修改或重编译宿主即可安装、交互、取消、恢复并显示标准视图；据此估算余量 |
| P4.7C 内置插件迁移 | 抽回 Core 职责；Codex/Pi 迁为独立插件；泛化模型、命令、树、审批和 Goal 等入口 | 内置与外部插件走同一路径，Phase 4.5/4.6 能力回归通过，生产业务路径不再按 Codex/Pi 分派 |
| P4.7D 视图与生命周期补全 | 差异化视图、两套皮肤、依赖诊断、版本并存、升级/卸载、历史兼容 | 皮肤切换保留状态，异常包与不兼容版本可解释，卸载后历史可读 |
| P4.7E 验收与开发者交付 | 插件开发文档、样例包、协议测试 harness、迁移与恢复证据 | 完成下述门禁，Phase 5 可仅依赖统一会话和能力契约 |

上述切片均未实施；内部适配层仅为迁移手段，不能作为内置插件迁移的最终验收结果。
