...
> 状态：实施中（P4.7A 契约与设计已完成；P4.7B 待实施）
...
| P4.7B 最小外部插件闭环 | 待实施 | Registry、Supervisor、统一会话 API、最小 Renderer、本地安装；现有 Manager 暂经内部适配层路由 | 仓库外测试插件无需修改或重编译宿主即可安装、交互、取消、恢复并显示标准视图；据此估算余量 |
