...
P4.7D 已开始实施生命周期与视图补全：插件安装记录现在保留已卸载墓碑；卸载先关闭该 Plugin Release 的活跃会话和进程，再移除包文件，同时保留 session、timeline、event、view 与固定 release 关联。相同 release 可按原 installation ID 重装，且重装后默认禁用。声明式 binding 与无需二次确认的扩展 action 已接通 `operation.invoke`：宿主核对固定 release、当前 generation、manifest、握手能力及输入/输出 schema，拒绝任意方法转发。依赖诊断、标准 action、受控资源和完整升级验收仍待后续切片完成。
