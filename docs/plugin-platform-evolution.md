...
宿主在 action 提交时再验证上下文、revision、generation 和权限，renderer 不直接选择任意 IPC method。P4 已接入非 `never` view action 的原生宿主确认和确认后上下文复核；调用者 revision/generation 校验和绑定发起窗口的原生对话框已通过验收，不能把显示按钮视为完整审批验收。详见 [P4 实施记录](./plugin-platform-p4-presentation.md)。
