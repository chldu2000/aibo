...
`App.svelte` 直接持有窗口标题栏、插件管理、执行历史、会话历史、设置和诊断；这些组件位于
...
任务与 Git 记录，按时间/类型/ID 的稳定游标翻页；关闭只释放读取订阅，停止请求使用原工作区/run ID，Git
仍由宿主核对调用窗口。会话历史使用只读 Core 消息分页接口，包含归档会话，选择不改变工作台会话；Broker 审计浏览仍待接入。任一来源读取失败时禁止向更早处推进游标，避免遗漏记录。
