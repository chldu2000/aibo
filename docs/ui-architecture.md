...
- 仅 Git 变更文件行使用 32px 高度，文件状态徽标为 18px 方形；文件名完整优先，目录从左侧截断。
  工作区、会话行保持原有 44px 最小高度，Git 提交历史保持两行信息及原有行距，不套用紧凑文件行规则。
...
- 消息元信息 12px；工具组是无内边距的带边框列表，摘要行 44px、13px、左右 12px。
...
- 发送、头部工具、分区刷新等控件保持真实 44px，因此工具条与分区标题比设计稿的 32px 控件各高约 8–12px；
  这是命中区边界的一部分，不再逐项列出。
...
Git 的分支摘要、分组标题、文件内容分别使用不同底色，选中页签有底色与下划线；
