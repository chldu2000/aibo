...
归档日期：2026-09-12。下文保留归档前的阶段叙述，不代表当前运行架构。
...
The first supported runtimes are Codex and Pi. The product architecture and phased delivery plan are documented in [docs/aibo-research-and-delivery-plan.md](aibo-research-and-delivery-plan.md); the macOS-first freeze is recorded in [docs/architecture-freeze.md](architecture-freeze.md).

The proposed evolution toward a plugin host, capability plugins, and UI plugins is described in [docs/plugin-platform-evolution.md](plugin-platform-evolution.md).
...
[the migration record](../capability-session-migration.md).
