...
**当前进度：已完成。** [P4 退出核对](./plugin-platform-p4-exit-audit.md)已逐项对应当前代码、后端测试、浏览器与真实原生证据。默认工作台支持整体重排、专业呈现降级和独立恢复；任务、Git、通用 Capability 与语义视图写入已接入宿主批准、持久去重、取消和结果历史。验收范围为当前可信构建的 UI、两套皮肤与 macOS 原生流程；通用 Capability 写入按 Unix 开放，不据此宣称 Windows/Linux 桌面均已验收。后续进入 P5 的 SDK、仓库外样例和发布兼容验证。分批历史记录仍保留在[呈现边界实施记录](./plugin-platform-p4-presentation.md)。
...
- [x] 验证新增标准语义贡献无需逐一修改每套 UI；通用导航保证入口可达，缺提供者或不兼容有诊断。见 [P4 退出核对：标准贡献与独立宿主入口](./plugin-platform-p4-exit-audit.md)及两次真实 App 启动的安装/重启/卸载诊断证据。
...
- [x] 用实际专业呈现验证 fallback 保留必需数据、操作和选择规则，不嵌入其他皮肤组件；时间线与 Composer 暂保留可信专业实现。见 [P4 退出核对：实际专业呈现与数据保留](./plugin-platform-p4-exit-audit.md)，专业 detail 与核心 collection 的验证范围分别记录。
...
- [x] 确保插件管理、授权、历史、停用故障插件和默认工作台恢复入口独立可用，UI 不能遮蔽或伪造可信审批。
- [x] 先从 `lib.rs` 提取可测试的项目任务/Git 领域服务，再接受控执行；宿主保留批准、取消、日志、artifact、工作区限制和输出限制。
- [x] 接入项目任务和 Git 写入，覆盖重复提交、作用域写冲突、取消、超时结果未知与禁止盲目重试；通用宿主路由不新增 Provider 分支。
- [x] 开放通用 Capability 实际写入，接入权限、原始调用者/调用链约束、可信审批、持久幂等、取消和结果未知；完成审批预检执行边界的专项验收。
- [x] 验证写入执行状态独立于页面生命周期；离开页面不丢弃结果，基于失效上下文的写入重新验证条件与权限，必要时重新确认。
- [x] 完成整体布局切换、全部核心语义视图、键盘、焦点、屏幕阅读器标签、reduced-motion 与两套皮肤验收。见 [P4 退出核对：整体交互验收](./plugin-platform-p4-exit-audit.md)，包含本轮五组浏览器探针与 macOS 原生 AX/恢复复跑；跨平台发布支持仍由 P5 明确。
...
**当前进度：** 已提取[公共纯数据 SDK 与本地 Capability Runtime helper](./plugin-platform-p5-sdk.md)，宿主与打包消费者共用类型定义；仓库外插件已通过 macOS 实际 App 的安装/绑定/调用、双皮肤语义贡献与卸载验证。可信 renderer 本地接口包、发布兼容和升级恢复矩阵继续推进。
...
P1 消息/revision/交互/限额与 P2 窗口隔离、存储键/保存时机/保留期限已在阶段实施记录中冻结。P3 的命名空间校验、契约、只读调用链和实例资源策略已在阶段合同中落实。P4 的实际专业呈现协商与通用写入完整流程已按退出矩阵验收；P5 继续明确稳定协议支持窗口和弃用通知。状态归属与恢复原则、只读结果接纳方式已确认，不再列为未定方向。
