...
- [ ] 新会话明确显示 interaction mode、approval、filesystem、command、network 与 sandbox 状态。
- [ ] requested、enforced 和 unsupported 可区分，adapter 不会静默降级。
- [ ] Plan 模式不能写文件或执行有副作用命令。
- [ ] Codex workspace-write/on-request 在 macOS 真实会话中生效。
- [ ] Pi 通过 Aibo 受控工具完成写入和命令，UI 仍明确标注无原生沙箱。
- [ ] 拒绝审批、中止 turn、adapter crash 后不会继续执行待审批动作。
- [ ] 工作区外路径、`..`、绝对路径越界和 symlink 逃逸均被拒绝。

### G1：变更与审阅

- [ ] 每个终止态 turn 都有 baseline/result，采集失败有明确状态。
- [ ] Changes 可切换“本轮”和“整个工作区”。
- [ ] 文件新增、修改、删除、重命名、二进制和大文件显示正确。
- [ ] turn 前已有 dirty/untracked 内容不会被归因给 Agent。
- [ ] diff/hunk、命令 argv、cwd、退出码和 verification 可追溯到 session/turn。
- [ ] stage/unstage/revert 使用细粒度 Core command，失败不破坏工作区。

### G2：Checkpoint 与恢复

- [ ] Agent 修改文件前创建可验证 checkpoint。
- [ ] 恢复前可以预览影响范围并取消。
- [ ] 无冲突恢复覆盖新增、修改和删除场景。
- [ ] 用户后续修改造成 hash 不一致时默认拒绝恢复。
- [ ] 恢复中断或失败不会产生不可解释的半恢复状态。
- [ ] 应用重启后 checkpoint、artifact 和 change set 仍可读取。

### G3：上下文、输出与任务控制

- [ ] 可以附加工作区内文件和目录，并在发送后保留结构化引用。
- [ ] 图片、模型和推理强度按 adapter capability 显示。
- [ ] 缺失、变化、超限和不支持的附件在发送前给出明确处理结果。
- [ ] Markdown、代码、diff、命令和测试结果安全且可操作。
- [ ] Agent 输出无法执行 HTML、脚本、危险协议或 Tauri IPC。
- [ ] planning/running/waiting approval/waiting user/completed/failed/interrupted 状态一致。
- [ ] Pi steer/follow-up 队列可查看和管理，重启后状态不伪造。

### G4：回归与持久化

- [ ] Phase 1–4 的会话、审批、Pi tree、归档、恢复和事件隔离测试继续通过。
- [ ] Codex/Pi 在同一工作区的并行会话不会串 profile、事件、artifact 或 checkpoint。
- [ ] 应用异常退出后，运行中 turn 被标为 interrupted，不被误报 completed。
- [ ] 旧数据库可迁移；历史会话默认保持只读兼容。
- [ ] 日志、数据库和 artifact 元数据不保存 auth token 或未脱敏 secret。
