...
> 本文是实施过程记录，各阶段的尺寸、状态和验证结论只代表当时范围；后续决定见 [Aibo ak-ui 现行规范](ak-ui-current-spec.md)。特别是 P2 的常态状态文案、P3 的 44px 命中区表述，均已被后续改动调整。
...
- `76b18d9`：会话仅活动执行显示圆环；正常状态只用图标颜色和时间，阻塞或异常状态显示标签。
- `4f6cef1`：Git 面板重排仓库、分支、页签、提交框和文件分组。
- `ded2d7e`、`fa242c8`：移除 Git 文件/历史行和普通按钮悬停时多出的下方信号条，保留选中信号与焦点反馈。
- `8e1c6b2`：补齐设计稿其余桌面细节，Git 文件行最终按 36px、状态标记按 18px；侧栏与两行历史保持各自的信息密度。
- `314e70e`：Git 仓库选择、分组标题、输入与紧凑按钮随文件行一起调整；32px 工具栏按钮用外扩命中区域。
- `d3c5aa6`：历史条目改为通栏双行、相对时间与细分隔线，长标题在时间前省略。

触屏 Git 文件行的命中区域，以及悬停或焦点才显现的分组批量按钮，仍需单独验收；不由桌面探针通过推定完成。
