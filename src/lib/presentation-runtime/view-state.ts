...
export type PresentationViewState = {
  timeline?: {key:string;offset:number;source:'default'|'external'} | null;
...
export function createPresentationViewStateStore(limit=32){
 ...
 function valid(value:unknown):value is PresentationViewState{
  ...
  return (v.timeline===undefined||v.timeline===null||Boolean(v.timeline&&key(v.timeline.key)&&number(v.timeline.offset)&&['default','external'].includes(v.timeline.source)))
   &&Array.isArray(v.window)&&v.window.length===2&&v.window.every(number)
 ...
 return {
  ...
  write(context:PresentationContext,value:unknown){if(!valid(value))return;const id=scope(context);states.delete(id);states.set(id,structuredClone({...(value.timeline===undefined?{}:{timeline:value.timeline?{key:value.timeline.key,offset:value.timeline.offset,source:value.timeline.source}:null}),focus:value.focus?{key:value.focus.key,selection:value.focus.selection}:null,scroll:value.scroll.map(({key,x,y})=>({key,x,y})),window:value.window,disclosures:value.disclosures.map(({key,open})=>({key,open}))}));while(states.size>limit)states.delete(states.keys().next().value!);},
  clear(){states.clear();},
 };
}
