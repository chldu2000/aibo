import { CORE_SEMANTICS, SUPPORTED_SNAPSHOT_SCHEMAS, type RendererDescriptor } from '../presentation/renderer-contract.ts';
...
export function validateRenderer(descriptor: RendererDescriptor): void {
  ...
  if (descriptor.snapshotSchemas !== undefined && (!Array.isArray(descriptor.snapshotSchemas) || !descriptor.snapshotSchemas.includes('aibo.semantic-view/v1') || new Set(descriptor.snapshotSchemas).size !== descriptor.snapshotSchemas.length || descriptor.snapshotSchemas.some(schema => !SUPPORTED_SNAPSHOT_SCHEMAS.includes(schema)))) throw Error('incompatible_renderer: snapshot schema declaration');
