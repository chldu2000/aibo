...
/**
 * Aibo-side Pi commands are deterministic actions on the embedded session,
 * rather than text sent to the model.
 */
...
export type ParsedPiCommand = {
  name: string;
  args: string;
};

export function parsePiCommand(input: string): ParsedPiCommand | null {
