import type { CapabilityHistoryEvents, CapabilityHistoryScope, CapabilityHistoryScopeItem, CapabilityHistoryScopes } from '../types';
...
export type CapabilityHistoryState = { scopes:CapabilityHistoryScopeItem[]; nextScopes:string|null; selected:CapabilityHistoryScope|null; page:CapabilityHistoryEvents|null; pageNumber:number; loadingScopes:boolean; loadingEvents:boolean; error:string|null };
export const emptyCapabilityHistory = ():CapabilityHistoryState => ({scopes:[],nextScopes:null,selected:null,page:null,pageNumber:1,loadingScopes:false,loadingEvents:false,error:null});
export function createCapabilityHistoryController(ports:{
  list(before:string|null):Promise<CapabilityHistoryScopes>;
  read(scope:CapabilityHistoryScope,before:string|null):Promise<CapabilityHistoryEvents>;
  ...
  async function read(view:View):Promise<void> {
    ...
    try {
      const page=await ports.read(scope,view.before);
      if(current!==view||view.revision!==revision)return;
      if(page.schema!=='aibo.capability-history-events/v1'||capabilityScopeKey(page.scope)!==capabilityScopeKey(scope))throw Error('调用记录与当前作用域不匹配');
  ...
  async function list(view:View,before:string|null):Promise<void>{
    ...
    try{
      const page=await ports.list(before);if(current!==view)return;
      if(page.schema!=='aibo.capability-history-scopes/v1')throw Error('不支持的调用目录版本');
  ...
  return {
    ...
    open():Promise<void>{const view:View={state:emptyCapabilityHistory(),revision:0,before:null,back:[]};current=view;publish(view);return list(view,null);},
