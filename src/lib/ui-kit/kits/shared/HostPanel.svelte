<script lang="ts">
  ...
  function keydown(event: KeyboardEvent) {
    if (event.defaultPrevented || [...document.querySelectorAll<HTMLElement>('.settings-overlay,.command-palette-overlay,[role="alertdialog"]')].some(node => node.getClientRects().length)) return;
