<script lang="ts">
  ...
  import type { UiAgentStatusMarkProps } from '../contract';
  ...
  let props: UiAgentStatusMarkProps = $props();
  const Component = $derived($activeUiKit.AgentStatusMark);
</script>

{#if scope && $externalPresentation?.package.release.manifest.surfaces?.includes('controls')}
  <ExternalControl control="AgentStatusMark" {props} presentation={$externalPresentation} {scope}>
    <Component {...props} />
  </ExternalControl>
