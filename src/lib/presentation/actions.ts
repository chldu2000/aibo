...
export function actionMessage(snapshot: Snapshot, actionId: ActionMessage['actionId'], itemId: string | null = null): ActionMessage {
  ...
  if ((actionId === 'inspect' || actionId === 'open-diff') && (snapshot.view.kind !== 'collection' || !snapshot.view.items.some(item => item.id === itemId))) throw new Error('invalid_selection');
  if (actionId !== 'inspect' && actionId !== 'open-diff' && itemId !== null) throw new Error('invalid_selection');
