...
<section aria-label="插件调用历史" data-ui-component="capability-history" aria-busy={state.loadingScopes||state.loadingEvents} class="capability-history">
      ...
      {#each state.page?.events??[] as event (event.sequence)}
        <Card as="article" aria-label={`调用记录 ${event.sequence}`}>
          ...
          <CardContent><p>{event.payload.type==='legacy_snapshot' ? `开始时间 ${text(event.payload.startedAt)}` : text(event.payload.occurredAt)} · 调用 {text(event.payload.invocationId)}</p>
            <details><summary>查看调用详情</summary><Textarea aria-label={`调用记录 ${event.sequence}详情`} value={JSON.stringify(event.payload,null,2)} readonly rows={8} /></details>
          </CardContent>
        </Card>
      {/each}
    </div>
