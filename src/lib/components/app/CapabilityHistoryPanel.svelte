<script lang="ts">
  ...
  import type { CapabilityHistoryScope } from '$lib/types';
  let {state,desktop,onSelect,onReload,onMoreScopes,onRefresh,onOlder,onNewer,onLatest,onBack}: {
    state:CapabilityHistoryState;desktop:boolean;onSelect:(scope:CapabilityHistoryScope)=>void;
...
<section aria-labelledby="capability-history-heading" data-ui-component="capability-history" aria-busy={state.loadingScopes||state.loadingEvents} class="capability-history">
  ...
  <p>显示当前窗口的调用接纳、启动和结束记录，可核对调用链与绑定版本。插件停用或原作用域删除后仍可读取。</p>
  ...
  <div class="history-body">
    <nav aria-label="调用历史作用域" class="history-scopes">
      ...
      {#if desktop&&!state.loadingScopes&&!state.scopes.length&&!state.error}<p>当前窗口暂无生命周期记录。</p>{/if}
    </nav>
    <div class="history-events">
      ...
      {#each state.page?.events??[] as event (event.sequence)}
        <Card as="article" aria-label={`调用记录 ${event.sequence}`}>
          <CardHeader><CardTitle>{text(event.payload.capability)}</CardTitle><Badge variant="outline">{text(event.payload.type)} · {text(event.payload.status)}</Badge></CardHeader>
          <CardContent><p>{text(event.payload.occurredAt)} · 调用 {text(event.payload.invocationId)}</p>
