...
export const DomPresentationAdapter: WebPresentationAdapter = {
  async mount(target, props) {
    ...
    const render = () => {
      ...
      for (const action of current.actions.filter(action => action.id !== 'inspect' && action.id !== 'open-diff')) button(action.id, action.label);
      if (current.view.kind === 'collection') {
        ...
        for (const item of current.view.items) {
          ...
          button(current.actions.find(action => action.intent === 'inspect')?.id ?? 'inspect', `${current.actions.find(action => (action.id === 'inspect' || action.id === 'open-diff'))?.label ?? '查看差异'} ${text}`, item.id);
        }
      } else {
        ...
        const pre = doc.createElement('textarea'); pre.readOnly = true; pre.rows = 16; pre.setAttribute('aria-label', current.view.kind === 'detail' ? '文件差异内容' : '视图内容'); pre.value = current.view.content; section.append(pre);
        if (current.view.truncated) { const text = doc.createElement('p'); text.textContent = '内容已截断'; section.append(text); }
      }
