<script lang="ts">
  ...
  import { createInstalledController, type InstalledContribution, type InstalledPort } from '../presentation/installed-controller';
  ...
  let { workspaceId, contribution, port, onClose, stateStore = createViewStateStore() }: { workspaceId: string; contribution: InstalledContribution; port: InstalledPort; onClose: () => void; stateStore?: ViewStateStore } = $props();
  ...
  let restoring = false;
  let ticket = 0;
  const scope = () => ({ workspaceId, contributionId: `${contribution.contributionId}.${contribution.installationId}` });
  function save() {
  ...
  async function restore() {
    ...
    await controller.open(workspaceId, contribution);
    ...
    if (value?.view.kind === 'collection') {
      ...
      value.view.selection = focusTarget;
      if (focusTarget && remembered.detail === focusTarget) await controller.act(actionMessage(value, 'open-diff', focusTarget));
    }
    if (currentTicket === ticket) { restoring = false; save(); }
  }
  $effect(() => { workspaceId; contribution; untrack(() => { void restore(); }); });
