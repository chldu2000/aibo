...
1. 将本目录复制到仓库外。
2. 将本地打包的两个 SDK tarball 安装为开发依赖；使用 TypeScript 编译器运行 `tsc -p tsconfig.json`。当前 SDK 尚未发布注册表，不能假设公网安装已可用。
3. 清单通过 `hostSdk` 选择宿主 SDK，使用 `npm pack --ignore-scripts` 后产物不包含这两个 SDK。最终 package.json 的开发依赖使用版本号，不保留本机 tarball 路径。参见[宿主 SDK](../../docs/host-sdk.md)。
